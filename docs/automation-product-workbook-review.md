...
| Home maintenance errors use user-facing automation names | Missing automation could surface handler names/timestamps instead of what the user needs to fix | L2 trust copy | Home shows daily refresh, afternoon reminder, weekly review, or edit actions/popups instead of function names |
...
| Setup menu actions pass the visible-action test | The old trigger/status labels exposed implementation and one Today action promised Today-only columns while showing all hidden columns | L2 trust/navigation copy | Menu says Setup & automation, Turn on Planner actions, Check Planner setup status; the misleading Today-only hidden-column action is removed |
| Row action labels say what they actually create or route | Several row actions used queue/prep/contact language that hid whether the result was a task, decision, planning route, or existing-record link | L2/L4 navigation copy | Labels now distinguish create task, ask whether, start application plan, and link existing contact |
